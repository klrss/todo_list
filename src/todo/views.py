...
def index(request):
    ...
    if request.method == 'POST':
        ...
        task_ex = Todo.objects.values_list('task', flat=True) #list of todo objects
        if form.is_valid() and request.POST['task']  not in task_ex:
            form.save()
        else:
            messages.add_message(request,messages.WARNING, 'The "{}" task is already added.'.format(request.POST['task']))
